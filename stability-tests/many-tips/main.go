...
func startNode() (teardown func(), err error) {
	...
	dataDir, err := common.TempDir("kasv2d-datadir")
	if err != nil {
		panic(errors.Wrapf(err, "Error in Tempdir"))
	}
	log.Infof("kasv2d datadir: %s", dataDir)

	kaspadCmd, err := common.StartCmd("KASV2D",
		"kasv2d",
	...
	spawn("startNode-kaspadCmd.Wait", func() {
		...
		if err != nil {
			if atomic.LoadUint64(&shutdown) == 0 {
				panics.Exit(log, fmt.Sprintf("kasv2dCmd closed unexpectedly: %s. See logs at: %s", err, dataDir))
			}
			if !strings.Contains(err.Error(), "signal: killed") {
				// TODO: Panic here and check why sometimes kaspad closes ungracefully
				log.Errorf("kasv2dCmd closed with an error: %s. See logs at: %s", err, dataDir)
			}
		}
	...
	return func() {
		...
		killWithSigterm(kaspadCmd, "kasv2dCmd")
