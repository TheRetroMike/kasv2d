...
func startNodes() (teardown func(), err error) {
	...
	syncerDataDir, err := common.TempDir("kasv2d-datadir-syncer")
	if err != nil {
		panic(errors.Wrapf(err, "error in Tempdir"))
	}
	log.Infof("SYNCER datadir: %s", syncerDataDir)

	syncedDataDir, err := common.TempDir("kasv2d-datadir-synced")
	if err != nil {
		panic(errors.Wrapf(err, "error in Tempdir"))
	}
	log.Infof("SYNCED datadir: %s", syncedDataDir)

	syncerCmd, err := common.StartCmd("KASV2D-SYNCER",
		"kasv2d",
	...
	syncedCmd, err := common.StartCmd("KASV2D-SYNCED",
		"kasv2d",
		common.NetworkCliArgumentFromNetParams(activeConfig().NetParams()),
