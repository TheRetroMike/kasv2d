...
func commandLoop(argsChan <-chan []string) ([]commandFailure, error) {
	...
	dataDirectoryPath, err := common.TempDir("kasv2dsanity-kaspad-datadir")
	if err != nil {
	...
	for args := range argsChan {
		...
		cmd := exec.Command("kasv2d", args...)
		cmd.Stdout = common.NewLogWriter(log, logger.LevelTrace, "KASV2D-STDOUT")
		cmd.Stderr = common.NewLogWriter(log, logger.LevelWarn, "KASV2D-STDERR")
