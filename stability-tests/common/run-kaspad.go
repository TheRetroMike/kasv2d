...
func RunKaspadForTesting(t *testing.T, testName string, rpcAddress string) func() {
	...
	kaspadRunCommand, err := StartCmd("KASV2D",
		"kasv2d",
	...
	t.Logf("Kasv2d started with --appdir=%s", appDir)
	...
	go func() {
		...
		if err != nil {
			if atomic.LoadUint64(&isShutdown) == 0 {
				panic(fmt.Sprintf("Kasv2d closed unexpectedly: %s. See logs at: %s", err, appDir))
			}
		}
	}()
